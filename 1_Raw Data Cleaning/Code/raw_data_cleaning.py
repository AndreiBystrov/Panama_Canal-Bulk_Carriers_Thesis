...
# Sort the table by 'O-D Region' in custom order: O-D, D-O, and so on
def custom_sort_key(od_region):
    """Creates a custom sort key for OD regions."""
    parts = od_region.split('_to_')
    if len(parts) == 2:
        # Sort the parts alphabetically and join them back
        return '_to_'.join(sorted(parts))
    return od_region

# Apply the custom sort key to the 'O-D Region' column
new_table_sorted['sort_key'] = new_table_sorted['O-D Region'].apply(custom_sort_key)

# Sort by the custom sort key and then by 'O-D Region'
new_table_sorted = new_table_sorted.sort_values(by=['sort_key', 'O-D Region']).drop(columns='sort_key').reset_index(drop=True)
...
# Distance 94 vs Shortest DIstance
# Prepare the data for plotting
melted_distance = Bulk_PA.melt(
    id_vars=['region_pairs'],
    value_vars=['distance_94', 'shortest_distance_nm'],
    var_name='Metric',
    value_name='Value'
)

# Sort the unique region pairs alphabetically for consistent plotting
sorted_region_pairs = sorted(melted_distance['region_pairs'].unique())

# Create the box plot
plt.figure(figsize=(18, 8)) # Adjust figure size as needed
sns.boxplot(x='region_pairs', y='Value', hue='Metric', data=melted_distance, order=sorted_region_pairs)

plt.title('Distance_94 vs. Shortest Distance by Region Pair')
plt.xlabel('Region Pair')
