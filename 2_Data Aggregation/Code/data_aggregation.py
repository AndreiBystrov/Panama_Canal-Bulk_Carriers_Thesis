...
# Convert to meters (as other variables are given in meters)
...
# Add commodity price information to Bulk_PA
for index, row in Bulk_PA.iterrows():
    year_month = row['proxy_transit_month']
...
# Fill the NaN values in 'cargo_value' with the calculated mean - gives the mean frequency
...
# Check if there are any remaining NaN values in the column
...
# Round vessel dimensions to make it more realistic. No vessel will be 10 cm larger than the Panamax locks, \
...
# Extract the final dataset for statistical modeling
...
# Filter data during El Niño (June 2023 to August 2024)
...
# Group & aggregate data to fit the statistical model needs
